...
2026-09-02，最初的故障电脑确认 NVIDIA `616.56` 已恢复正常的原生息屏，不再需要本项目的 workaround：

| 驱动 | 单屏 | 双屏 | 自定义分辨率 | Workaround |
| --- | --- | --- | --- | --- |
| `610.88`（R610） | 亮黑色背光 | 亮黑色背光 | 无需创建即可复现 | 未运行 |
| `616.56`（R615） | 正常息屏 | 两台显示器均正常息屏 | 没有创建 | 未运行 |

`610.88` 和 `616.56` 是相邻的公开 GeForce WHQL 版本，因此 `616.56` 是目前确认在 HKC C340 / RTX 3070 Ti DisplayPort 测试环境中修复该问题的第一个正式驱动。NVIDIA 的 [616.56 发行说明](https://us.download.nvidia.com/Windows/616.56/616.56-win11-win10-release-notes.pdf) 明确表示公开项目只是驱动全部改动的一部分，但没有列出本次息屏修复，也没有提供对应的公开 bug 编号。

项目因此进入维护状态。在 `616.56` 上应优先使用 Windows 原生息屏；除非能够重新复现故障，否则不应安装 workaround。本项目继续保留，用于旧版受影响驱动以及未来驱动回归时的诊断。版本号高于 `616.56` 并不自动代表一定正常，仍应独立验证。
